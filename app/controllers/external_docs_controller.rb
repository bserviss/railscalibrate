class ExternalDocsController < ApplicationController
  ...
  def create
    @external_doc = ExternalDoc.new(params[:external_doc])
  ...
  def update
    ...
    respond_to do |format|
      if @external_doc.update_attributes(params[:external_doc])
        format.html { redirect_to(@external_doc, :notice => 'External doc was successfully updated.') }
        format.xml  { head :ok }
      else
