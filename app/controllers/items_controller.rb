...
class ItemsController < ApplicationController
  ...
  def index
    ...
    @events = Event.order( 'created_at DESC' ).limit( 10 ).all
  ...
  def show_thirty_days
    @display_group = Item.due_in( 30 ).paginate :page => params[:page], :per_page => 10
  ...
  def show_all_items_by_location
    @items = Item.all( :order => :location ).paginate :page => params[:page], :per_page => params[:per_page], :per_page => 15
  end
end